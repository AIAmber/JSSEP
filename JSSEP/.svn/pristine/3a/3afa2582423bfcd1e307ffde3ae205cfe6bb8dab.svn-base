require(['jquery','bootstrap-multiselect','bootstrap-switch','bootstrap-grid','Util','bootstrap-dialog','daterangepicker','daterangepicker-zh-CN'], function($,bootstrapmultiselect,bootstrapswitch,grid,Util,BootstrapDialog,datetimepicker) {
	...
	//表单校验规则
	a = Util.validate("catalogtoberegistration",{
		ignore : ":hidden",
		rules: {
			classcify: {
				required: true
        ...
        buttons: [{
            ...
            action: function(dialogRef){
            	...
            	if(selectData.length>0){
					...
					$("#catalogtoberegistration").validate().element($('#resourceprovider'));
					dialogRef.close();
				}else{
	...
	var catalogfailreasonone = $("#catalogfailreasonone");
	//注册失败按钮弹出框
	var failDialog = new BootstrapDialog({
		title: '退回原因',
		type:'type-default',
        message: $("#catalogfailreasonone"),
        closeByBackdrop: false,
        closeByKeyboard: false,
        draggable: true,
        autodestroy:false,
        cssClass:'resourcefail-dialog',
        buttons: [{
            label: '确定',
            autospin: false,
            icon: 'glyphicon glyphicon-check',    
            cssClass:'btn-success btn-sm',
            action: function(dialogRef){
            		if(catalogfailreasonone.val().length>0){
            			//获取注册失败原因文本内容保存到后台
            			failDialog.close();
            			Util.promptSuccessDialog("目录注册失败!");
            			location.href="#/catalogregistration";
            		}else{
            			Util.warningDialog("请输入退回原因！");
            		}
            }
        },{
            label: '取消',
	...
	//表单 不通过按钮事件
	$('#catalogregistrationfail').on('click',function(){
		failDialog.open();
	});
	
	//表单通过按钮事件
	$("#saveForm").click(function(){
		var flag = $("#catalogtoberegistration").valid();
		if(flag==false){
			a.focusInvalid();
		}else{
			Util.questionDialog('确定提交？',function(dialog, result){
				if(result){
					//ajax 提交数据到后台
					Util.promptSuccessDialog("目录注册成功！");
					location.href="#/catalogregistration";
				}
				dialog.close();
			});
	...
	$("#revokebtn").click(function(){
		location.href="#/catalogregistration";
		/*Util.questionDialog('是否返回到目录注册列表？',function(dialog, result){
			if(result){
//				Util.dangerDialog("保存编目信息成功!");
	...
	$("#resetForm").click(function(){
		Util.questionDialog('确定重置？',function(dialog, result){
			if(result){
				Util.validate("catalogtoberegistration",{}).resetForm();
				$("#resourceformatchoice").children().empty();
				$("#infoItemTable").children().empty();
				//清空表单
				$("#catalogtoberegistration")[0].reset();
			}else{
