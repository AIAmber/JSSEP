...
require(['jquery','bootstrap-multiselect','bootstrap-switch','Util','bootstrap-dialog','daterangepicker','daterangepicker-zh-CN'], function($,bootstrapmultiselect,bootstrapswitch,Util,BootstrapDialog,datetimepicker) {
	...
	//给标签增加属性，使表单只读
	$("#catalogalreadyregistration input").attr("disabled","disabled");
	$("#catalogalreadyregistration select").attr("disabled","disabled");
	$("#catalogalreadyregistration textarea").attr("readonly",true);
	...
	$("#saveForm").click(function(){
		//$.ajax();
		Util.questionDialog('确定保存目录注册信息？',function(dialog, result){
			if(result){
//				Util.dangerDialog("保存编目信息成功!");
	...
	$("#revokebtn").click(function(){
		location.href="#/catalogregistration";
		/*Util.questionDialog('确定返回到目录注册列表？',function(dialog, result){
			if(result){
//				Util.dangerDialog("保存编目信息成功!");
				location.href="#/catalogregistration";
			}
