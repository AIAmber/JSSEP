...
require(['jquery','bootstrap-multiselect','bootstrap-switch','Util','bootstrap-dialog','daterangepicker','daterangepicker-zh-CN'], function($,bootstrapmultiselect,bootstrapswitch,Util,BootstrapDialog,datetimepicker) {
	...
	//给标签增加属性，使表单只读
	$("#catalogalreadycheck input").attr("disabled","disabled");
	$("#catalogalreadycheck select").attr("disabled","disabled");
	$("#catalogalreadycheck textarea").attr("readonly",true);
	...
	$("#saveForm").click(function(){
		//$.ajax();
		Util.questionDialog('确定保存目录审核信息？',function(dialog, result){
			if(result){
//				Util.dangerDialog("保存编目信息成功!");
	...
	$("#revokebtn").click(function(){
		location.href="#/catalogcheck";
		/*Util.questionDialog('确定返回到目录审核列表？',function(dialog, result){
			if(result){
//				Util.dangerDialog("保存编目信息成功!");
				location.href="#/catalogcheck";
			}
